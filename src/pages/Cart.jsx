    ...
    <div className="p-4 max-w-4xl mx-auto">
      ...
      {cartItems.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
          ...
          <div className="px-2">
            {cartItems.map(item => (
              <CartItem key={item.id} item={item} updateQuantity={updateQuantity} removeFromCart={removeFromCart} isChecked={selectedIds.includes(item.id)} onToggle={() => handleToggleCheckbox(item.id)} />
            ))}
          </div>
        </div>
      )}
      ...
      {cartItems.length > 0 && (
        <div className="sticky bottom-4 z-40 bg-white border border-gray-200 p-4 rounded-xl shadow-lg flex flex-col md:flex-row justify-between items-center gap-4 text-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              onClick={() => selectedIds.length > 0 && setIsPurchaseModalOpen(true)}
              disabled={selectedIds.length === 0}
              className={`px-6 py-2 rounded font-medium text-white shadow ${selectedIds.length === 0 ? 'bg-gray-300' : 'bg-blue-600 hover:bg-blue-700'}`}
            >
              Mua hàng
            </button>
          </div>
      ...
      {isPurchaseModalOpen && (
        <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-[9999]">
          <div className="bg-white p-6 rounded-xl shadow-xl w-[90%] max-w-sm text-center">
            ...
            <p className="text-gray-500 mb-5 text-sm">Tổng tiền: <strong className="text-red-500">{selectedTotal.toLocaleString('en-US')} đ</strong></p>
            <div className="flex gap-2 justify-center">
              <button onClick={() => setIsPurchaseModalOpen(false)} className="px-4 py-1.5 bg-gray-100 text-gray-800 rounded text-sm font-medium">Hủy</button>
              <button onClick={handleConfirmPurchase} className="px-4 py-1.5 bg-green-500 text-white rounded text-sm font-medium">Đồng ý</button>
            </div>
          </div>
        </div>
      )}
      {isSuccessModalOpen && (
        <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-[9999]">
          <div className="bg-white p-6 rounded-xl shadow-xl w-[90%] max-w-sm text-center">
            <h3 className="text-lg font-bold text-green-600 mb-2">✓ Thành công!</h3>
            <p className="text-gray-500 mb-5 text-sm">Đơn hàng của bạn đang được xử lý.</p>
            <button onClick={() => setIsSuccessModalOpen(false)} className="w-full py-2 bg-blue-600 text-white rounded text-sm font-medium">
              Tiếp tục mua sắm
            </button>
          </div>
        </div>
