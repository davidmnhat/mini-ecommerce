            ...
            <div className="relative group py-2">
              ...
              <div className="absolute right-0 top-full mt-1 w-80 bg-white rounded-xl shadow-2xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 flex flex-col">
                <div className="p-4 border-b border-gray-100 bg-gray-50 rounded-t-xl">
                  <p className="font-bold text-gray-800">{user.username || user.name}</p>
                  <p className="text-xs text-gray-500">Thành viên TechShop</p>
                </div>
                
                <div className="max-h-72 overflow-y-auto p-2">
                  <p className="text-xs font-bold text-gray-400 mb-2 px-2 uppercase tracking-wider">Đơn hàng đã đặt</p>
                  
                  {(() => {
                    const orders = JSON.parse(localStorage.getItem(`orders_${user.id}`)) || [];
                    if (orders.length === 0) {
                      return <p className="text-sm text-gray-500 px-2 py-4 text-center italic">Chưa có đơn hàng nào.</p>;
                    }
                    return orders.map((order, index) => (
                      <Link to={`/order/${order.id}`} key={index} className="block p-3 border-b border-gray-50 last:border-0 hover:bg-blue-50/50 rounded-lg transition-colors mb-1 cursor-pointer">
                ...
                <div className="p-2 border-t border-gray-100 bg-gray-50 rounded-b-xl">
                  <button 
                    onClick={logout} 
                    className="w-full text-left px-4 py-2.5 text-sm text-red-600 font-bold hover:bg-red-100 rounded-lg transition-colors flex items-center gap-2"
                  >
